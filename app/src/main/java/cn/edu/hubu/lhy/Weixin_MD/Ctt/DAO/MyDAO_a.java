...
/*
    ...
    删除记录的方法：deleteInfo(int selId)
    修改记录方法：updateInfo(String name,int age,int selId)
*/
public class MyDAO_a {
    private SQLiteDatabase myDb;  //类的成员
    private DbHelper dbHelper;  //类的成员

    public MyDAO_a(Context context) {  //构造方法，参数为上下文对象
        //第1参数为上下文，第2参数为数据库名
    ...
    public void deleteInfo(int selId){  //删除记录
    ...
    public void updateInfo(String name,String tel,int selId){  //修改记录
        //方法中的第三参数用于修改选定的记录
        ContentValues values = new ContentValues();
        values.put("name", name);
        values.put("tel", tel);
